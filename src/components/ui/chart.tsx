...
/* ==== Recharts-kompatibilní minimální typy (fungují napříč verzemi) ==== */

type RPValue = number | string | null | undefined;
type RPName = string | number | null | undefined;
...
type TooltipLikeProps = {
  active?: boolean;
  payload?: ReadonlyArray<RechartsPayload>;
  label?: React.ReactNode;
  labelFormatter?: (
  ...
  formatter?: (
    ...
    index: number,
    rawPayload: RechartsPayload["payload"]
  ) => React.ReactNode;
};
  ...
  const css = Object.entries(THEMES)
    .map(([theme, prefix]) => {
      const lines = colorConfig
        .map(([key, itemCfg]) => {
          const color = itemCfg.theme?.[theme as keyof typeof itemCfg.theme] ?? itemCfg.color;
      ...
      <div className="grid gap-1.5">
        {list.map((item, index) => {
          ...
          const indicatorColor =
            color || (item.payload && (item.payload as Record<string, unknown>)["fill"]) || item.color;
...
function getPayloadConfigFromPayload(
  config: ChartConfig,
  payload: RechartsPayload | LegendItem,
  key: string
) {
  // hlavní objekt (toolip/legend) i případné vnořené payload
  const raw =
    (payload as RechartsPayload).payload ??
    (payload as LegendItem).payload ??
    payload;
  ...
  if (
    ...
    configLabelKey = (raw as Record<string, unknown>)[key] as string;
  }
