...
interface FullGardenViewProps {
  habits?: HabitView[];            // <= může přijít undefined
  theme: 'day' | 'night';
}
...
export function FullGardenView({ habits = [], theme }: FullGardenViewProps) {
  ...
  const totalProgress = safe.reduce((acc, habit) => {
    const stageValues = { seed: 25, sprout: 50, flower: 75, tree: 100 } as const;
    return acc + stageValues[habit.stage];
  }, 0);
  const averageProgress = safe.length > 0 ? Math.round(totalProgress / safe.length) : 0;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {safe.length} {safe.length === 1 ? 'plant' : 'plants'} growing • {averageProgress}% average growth
            </p>
          </div>
          ...
          {safe.length > 0 ? (
            <div className="relative h-[280px]">
              <ImageWithFallback
                ...
                alt="Growing garden"
                className={`w-full h-full object-cover transition-all duration-500 ${isDark ? 'opacity-40 grayscale' : 'opacity-70'}`}
              />
              <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-t from-slate-900 via-slate-900/50 to-transparent' : 'bg-gradient-to-t from-green-50/80 via-transparent to-sky-100/40'}`} />
