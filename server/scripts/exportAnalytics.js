...
async function main() {
    ...
    for (const u of users) {
        ...
        for (const t of ticks) {
            if (t.dayKey)
                uniqueDays.add(String(t.dayKey));
            const ts = toTs(t.createdAt || t.dayKey);
            if (ts !== null) {
                if (firstTickTs === null || ts < firstTickTs)
                    firstTickTs = ts;
                if (lastTickTs === null || ts > lastTickTs)
                    lastTickTs = ts;
            }
        }
        ...
        rows.push({
            ...
            avgHabitStreak: habits.length > 0
                ? (habits.reduce((sum, h) => sum + (h.streak ?? 0), 0) / habits.length).toFixed(2)
                : "0",
            // globální XP / streak
            xp: u.xp ?? 0,
            level: u.level ?? 1,
            ...
            ticksPerActiveDay: daysActive > 0 ? (totalTicks / daysActive).toFixed(2) : "0",
            // experiment
            variantSwitchCount,
